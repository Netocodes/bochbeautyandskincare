...
export const Navbar = () => {
  ...
  return (
    <div className="bg-black/10">
      <div className="px-2 fixed top-0 mx-auto w-full  md:backdrop-blur-lg">
        <nav className="bg-black/75 z-[50]  py-4 w-full md:max-w-[76rem] rounded-lg my-3 md:rounded-full mx-auto flex items-center justify-between px-8">
          ...
          <div
            className={
              isOpen
                ? " bg-black/90 fixed flex flex-col  gap-y-12 md:hidden left-0 top-0 w-[80%] h-full border-r border-r-gray-900  ease-in-out duration-500"
                : "  ease-in-out w-[80%] duration-500 fixed top-0 bottom-0 left-[-100%]"
            }
          >
            <div className="bg-black flex flex-col gap-y-12">
              {/* Mobile Logo */}
              ...
              <div className="bg-black">
                {navArray.map((item) => (
                  <ul className="text-white text-xl gap-y-4 ">
                    <li
                      key={item.id}
